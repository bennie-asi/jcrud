package xyz._990904.Jcrud.manager.common;
...
import xyz._990904.Jcrud.command.PageResultCommand;
import xyz._990904.Jcrud.command.SaveOrUpdateDataCommand;
import xyz._990904.Jcrud.dao.common.BaseCRUDMapper;
import xyz._990904.Jcrud.utils.ColumnUtils;
import xyz._990904.Jcrud.utils.FieldUtils;
...
public class BaseCRUDManagerImpl implements BaseCRUDManager {
     ...
     * 根据实体对象的非Null字段作为Where条件查询结果集，如果对象的属性值都为null则返回全部数据等同于selectAll+分页
     *
     * @param t 实体对象(如果要支持分页实体对象可以继承xyz._990904.Jcrud.command.BaseModel)
    ...
    private <T> void buildFieldValue(Map<String, Object> map, T newInstance, Field field, String name) throws IllegalAccessException {
        if (field.getGenericType().toString().equals("class java.sql.Timestamp") && map.get(name) != null) {
            field.set(newInstance, Timestamp.valueOf(map.get(name).toString()).toLocalDateTime());
        } else if (field.getGenericType().toString().equals("class java.time.LocalDateTime") && map.get(name) != null) {
            field.set(newInstance, LocalDateTime.parse(map.get(name).toString()));
